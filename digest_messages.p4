...
control MyIngress(inout headers hdr,
    ...
    // Read the threshold from flow rules
    action rcv_threshold( bit<32> threshold) {
        meta.PACKET_THRESHOLD = threshold;
    }
    ...
    action ipv4_forward(macAddr_t dstAddr, egressSpec_t port) {
       ...
       //set the destination mac address that we got from the match in the table
        ...
        // Set digest message values
        meta.learn.digest = meta.counter_one;
       ...
       thresh.apply();
        if (hdr.ipv4.isValid()){
            if (hdr.tcp.isValid()){
                ...
                if ( (meta.counter_one > meta.PACKET_THRESHOLD) ){
                    drop_table.apply();
                    return;
                }
